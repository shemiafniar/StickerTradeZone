...
import { removeDuplicateAction, updateDuplicateListingAction, type UpdateListingState } from "@/lib/actions/stickers";
...
export function DuplicateChip({ item }: { item: DuplicateWithNumber }) {
  ...
  if (open) {
    return (
      <form action={formAction} className="w-full max-w-xs rounded-xl border border-brand/30 bg-brand/5 p-3">
        ...
        <p className="mb-2 text-sm font-bold">עריכת מדבקה #{item.number}</p>
